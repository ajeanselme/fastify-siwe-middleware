...
test("DELETE /auth/logout invalidates current session when authorized", async () => {
	...
	expect(sessionService.invalidate).toHaveBeenCalledWith("session-123");

	await app.close();
});

test("DELETE /auth/logout returns 401 when token is missing", async () => {
	const app = await buildApp();
